...
- [ ] Build it: `/develop abstention verification reliability`
  ...
  - [ ] Settle spec 0010's three open decisions (inline citation markers, the contaminated self corpus gate, the out of sync decomposition prompt), then run the cheap self corpus gate (AC-4, AC-5, AC-11, AC-12)
  - [ ] Calibrate the additive tolerance against that measurement, then rewrite `verify.md` and run `/check verify` and `/test` (AC-1 to AC-12)
