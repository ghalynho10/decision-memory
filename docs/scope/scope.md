...
- [ ] Build it: `/develop abstention verification reliability`
  ...
  - [ ] Make the gate able to see a wrong answer before anything is calibrated by it: state the directness exclusion to the coverage model, then add the co-located citation and abstention cause oracle, the manifest battery loader, and `evaluate --battery` (AC-15, AC-16, AC-17)
  - [ ] Rewrite `verify.md`, run `/check verify` and `/test`, then calibrate the additive tolerance against the re-measured figure (AC-1 to AC-17)
