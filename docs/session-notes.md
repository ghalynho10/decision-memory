...
- Round 3 `/check review` fixes on feature 11 (evaluate's LockError handling during the battery, the query 3 and assertion A oracle false positive fixes, the non-empty `--store`/`--records` warnings, the `--runs` validation reordering) were applied and committed (`0b5e1bf`) but not re-reviewed. Per the standing instruction to re-run `/check review` after a fix batch, a round 4 is still owed before treating this branch as independently verified.
