...
- Verification layer reliability (observed 2026-08-10 while capturing the README demo): the query verification boundary is stochastic. Against this repo's own index, a well-supported question ("why was the entry point discovery approach rejected for third party adapters?") had retrieval right every time but answered only ~2/9 runs; the rest abstained because the LLM entailment/coverage verdicts varied. Worse, an off-topic question ("why was a monorepo chosen for the mobile apps?") produced a plausible-sounding answer that passed verification despite being barely grounded. Root-cause hypothesis: the draft → containment → entailment → coverage chain is LLM-dependent and unmeasured. Seed evidence for feature 11 (eval harness): use these two cases as the first fixed questions.
