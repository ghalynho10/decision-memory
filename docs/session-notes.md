...
- Current branch is `evaluation-harness` (feature 11), not the `feature/` branch AGENTS.md's git convention prescribes. Feature 16 should be built on its own `feature/` branch; none created yet, and it now has an accepted spec (0010) waiting to be built.
