...
- PR #4 is open (multi-source-retrieval to main, feature 10). A fresh /check review on opus (2026-08-11, superseding the earlier same-day review) found the semantic top-24 boundary test at tests/test_retrieval_stages.py was tautological against a gate recorded as passing (verdict: Changes requested). Fixed this session: the boundary test now runs query_index for real against 26 tied-distance rows; the AC-9 embedding-provider trace gap (`QueryDependencies.embed` widened to accept attempts, threaded through every `_failed_result`/`_abstained_result` call site in query.py); and the query 1 live oracle (tests/test_query_live.py) reground per spec Follow-up 5 to assert the citation resolves to DM-0012's `decision.alternatives[*]` chunk, not just any DM-0012 citation (Follow-up 5 ticked in index.md). 438 unit tests, ruff, and mypy src all clean after the fixes. Still open, deliberately deferred: store-parity-masked-as-abstention on an empty filter match, invented diversity dispositions in a diversity-failure partial trace, missing AC-16 architecture import test, and dead QUERY_SCHEMA_VERSION constant (all recorded as minors/nits in docs/reviews/2026-08-11-multi-source-retrieval.md) — decide whether to address before merging or merge as is.
