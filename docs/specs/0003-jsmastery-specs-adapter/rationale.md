...
**What the unresolved mention count did in reality, and the third calibration**: the counting rule was written, shipped, and then measured against this repository's own `docs/specs/`, which is the honest test the first two calibrations never got. Adapting spec 0003 counted 4,748 mentions, 1,426 of them distinct. The cause is not a bug in resolution; it is that backticks in this corpus mark prose at least as often as they mark paths. This project quotes short phrases and whole sentences in single backticks, so splitting every span on whitespace turns ordinary words into path candidates that predictably fail to resolve. The most frequent were `the` at 259, `a` at 144, and `and` at 120. A warning reading "4,748 unresolved mentions" cannot be acted on, which defeats what **AC-6** existed for.

The shape test in step 7 is therefore the third calibration of this rule, and the history is worth stating plainly so the next person reads it as tuning rather than principle. The first was too narrow: an identifier heuristic that would have dropped real evidence such as `proxy.ts`, `AGENTS.md`, and every directory reference, which is why the original spec chose existence as the disambiguator and said so explicitly. The second, the shipped one, moved all the way to the other end and counted anything that failed to resolve, which is how 4,748 happened. The third narrows the count without touching extraction.

Measured on spec 0003 by running both rules over identical text through the shipped extraction, the count falls from 4,748 occurrences (1,426 distinct) to 107 occurrences (39 distinct), after the inline code delimiter fix that treats backtick runs of equal length as a span. Both pairs are stated because `unresolved_mention_count` reports occurrences, so 107 is the number a person actually sees in the warning; the distinct figures are the better measure of how much there is to read. The seven resolved evidence targets are `.`, `AGENTS.md`, `docs/scope/scope.md`, `docs/specs`, `docs/specs/0001-stack-and-architecture.md`, `docs/specs/0002-canonical-decision-record-schema.md`, and `tests`. Of the 39 still counted, several are documentation placeholders or example values rather than real drift (`NNNN-kebab-title/`, `docs/specs/<n>`, `.md`, and `manifest.json`), so the honest reading is that a short scan remains, not a firehose. Two variants were rejected against that measurement. Putting the shape test upstream in extraction, which reads as the simpler single filter, drops `tests` and the `` `lib/` `` case the acceptance criteria already pin, reproducing the first calibration's failure in milder form. Accepting any one to six characters after a dot as a file extension keeps `decision.chosen` in the count, because this spec's own vocabulary is full of dotted field names such as `consequences.negative`; a known extension list removes that class at the cost of being a list someone maintains.
...
**Two corrections to the numbers above, and why they were wrong**: this paragraph first carried different figures, 4,999 falling to 34, with four evidence targets rather than five. They were wrong, and the fourth review round caught them. Both errors came from measuring with a script that re-implemented the extraction pipeline by hand instead of calling the shipped code. That replica diverged in two ways. It concatenated the two contributing files into one string, while the real `_evidence_and_unresolved` extracts from each file separately and sums, and with naive backtick pairing the join changes how spans pair across the seam. It also never counted the `spec` kind evidence, so it reported four file targets and missed that `docs/specs/0002-canonical-decision-record-schema.md` also resolves. A later review found the deeper cause, the inline code parser paired single backticks without respecting markdown delimiter length, so double backtick examples could hide later real spans. Fixing that parser changed the current reading to 107 unresolved occurrences, 39 distinct unresolved tokens, and seven file evidence targets. The wrong numbers survived a full cross check, because the check read the reasoning rather than re running the measurement.

The lesson is a sharper version of the one above and applies to this record too: validating a decision with a hand written stand in for the pipeline is the same mistake as tuning a heuristic against your own fixtures, and being aware of the first does not protect against the second. Any figure in this section should be reproduced with `JsmasteryAdapter().parse(...)` on the real corpus, never with a script that imitates it.

One more caveat that the correction exposed. The numbers here are measured against a spec that describes the adapter, so editing this spec changes the adapter's own input: adding quoted examples while documenting the rule moved the pre fix count from 3,611 to 4,748 without any code changing. Any figure recorded here is a reading taken at a moment, not a constant, and re measuring after an edit is expected to give a different answer.
