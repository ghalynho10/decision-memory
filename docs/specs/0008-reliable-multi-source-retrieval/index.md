...
13. **AC-13**: Against one rebuilt JobPilot index, the exact query `What decisions affect resume generation?` returns an answered, multi record result citing `DM-0004` and `DM-0019`. It states that resume generation runs on demand from the saved profile and stores the produced PDF, that projects are deliberately excluded from generated resumes, and that resume quality adds ATS guidance plus deterministic guards for unsupported numbers and em dash output. The first statement cites `DM-0004`; the projects exclusion and resume quality statements cite `DM-0019`. Each statement cites the record that supports it. Incidental mentions do not satisfy this oracle.
...
Query 2 oracle                  JobPilot DM-0004 and DM-0019 canonical chunks
...
1. Multi source happy path: query 2 accepts evidence from `DM-0004` and `DM-0019` and returns the two required cited decisions, verifies **AC-5** through **AC-13**.
...
5. [x] Reground the feature 9 live query 1 test (`test_query_one_against_real_jobpilot`) to assert the cited record and its structured content instead of generated prose: the current `"two agent routes" in joined` assertion pins model paraphrase, which hybrid retrieval can legitimately change. Separate from this oracle correction; reground it, do not loosen it. Regrounded 2026-08-11: asserts a citation resolves to `DM-0012`'s `decision.alternatives[*]` chunk, not just any DM-0012 citation.
...
8. [ ] Feature 11 carries three items from this feature's live gates: the query 4 fabrication (item 6), the query 5 expected abstention (item 1), and the query 2 `DM-0004` coverage omission observed in the 2026-08-11 verify run (cited in 3 of 5 runs before the AC-5 fusion boundary fix, and 0 of 5 in the post fix re run, always with `DM-0004` chunks in the diversity accepted context). The query 2 omission is the same verification gap class as query 4, not a retrieval bug; the rate moved with the retrieved context, which Feature 11's harness should measure.
9. [ ] The query 2 `DM-0004` rate moved from 5 of 5, to 3 of 5, to 0 of 5 across three samples, including a shift after the AC-5 fusion boundary fix changed the accepted context. This validates AC-15's own caveat that a five run smoke gate is not a reliability estimate. Feature 11's harness should measure the rate across runs rather than assume five consecutive passes hold.
10. [ ] A filter that matches no chunk returns the same honest abstention as a store parity failure or an over limit question with a nonmatching filter, because the token-limit and parity checks run after the empty-filter early return in `query_index` (`query.py`). Move both checks above the filter gate, or at minimum above the `if not accepted_ids` return, so store corruption cannot present as "not enough evidence here". Recorded as a minor in `docs/reviews/2026-08-11-multi-source-retrieval.md`.
11. [ ] `_fusion_stage` stamps every candidate with placeholder diversity fields (`breadth_disposition=RECORD_CAP`, `final_disposition=OUTSIDE_TOP_8`) meant to be overwritten by the diversity stage; when diversity raises, the partial trace's `FusionTrace` carries those unmodified placeholders as if diversity had run and rejected them, contradicting the "failing section and every later section are absent, not synthesized" promise. Have `_fusion_stage` return a pre-diversity value and only materialize `FusedCandidate` with real diversity fields after diversity succeeds. Recorded as a minor in `docs/reviews/2026-08-11-multi-source-retrieval.md`.
12. [ ] Critical test scenario 10 (AC-16, "an import check proves application has no third party retrieval import") has no test; the property currently holds by discipline and grep only. Add a test that walks `src/decision_memory/application` and `src/decision_memory/domain` with `ast.parse` and asserts no import of `rank_bm25`, `chromadb`, `openai`, `typer`, or `pydantic`. Recorded as a minor in `docs/reviews/2026-08-11-multi-source-retrieval.md`.
13. [ ] `QUERY_SCHEMA_VERSION` (`dto.py`) is defined and exported but never referenced; `schema_version=2` is hardcoded at three call sites in `query.py` with no test asserting `result.schema_version == 2`. Use the constant at all three sites and assert it in one query test, so a future schema bump has one place to change. Recorded as a minor in `docs/reviews/2026-08-11-multi-source-retrieval.md`.
