...
| The morphology rule family | `application/verification.py` `_stem_match` | `application/morphology.py` (new) | Two application callers, one named rule set |
...
Pinned as an exact algorithm rather than by its relationship to the pairwise matcher, because several non equivalent stemmers satisfy that relationship while producing different token streams and different rankings. Applied once per token, after stopword removal, as step 5 of `lexical-tokenizer-v2`.

1. If the token is shorter than `MIN_STEM_LENGTH` (3), return it unchanged. This mirrors the pairwise floor, which is measured on the untransformed shorter token, so a token of exactly 3 characters is reduced.
2. Strip the first matching suffix, longest first: `ing`, `ed`, `es`, `s`. Strip only if at least 2 characters remain. At most one suffix is stripped.
3. If the result ends in a doubled letter (its last two characters are equal), drop one.
4. Otherwise, if the result ends in `i`, replace that `i` with `y`.
5. Otherwise, if the result ends in `e`, drop it.
6. Return the result. Steps 3, 4, and 5 are mutually exclusive and apply at most one each; there is no second pass.

Normative examples, which double as the test vectors and as the evidence for what the family does and does not reach:

```text
Input        After step 2   Canonical stem   Note
decide       (none)         decid
decides      decid          decid            reaches the decide group
decided      decid          decid
deciding     decid          decid
decision     (none)         decision         does NOT reach decided, as intended
use          (none)         us
using        us             us               the e drop pair
ship         (none)         ship
shipped      shipp          ship             the doubling pair
rely         (none)         rely
relies       reli           rely             the y to i pair
records      record         record           ordinary plural
chose        (none)         chos
chosen       (none)         chosen           irregular, not reached by any stemmer here
retrieve     (none)         retriev
retrieval    (none)         retrieval        Porter would join these, this family does not
process      proces         proces           accepted over stripping, lexical side only
```
...
Verification stem match         the same family's pairwise entry point, behaviour unchanged
...
5. The morphology family has one definition. Two entry points may exist for the two shapes the callers need, and the agreement property is tested; two independent rule sets may not.
...
6. Morphology agreement: for the family's cases, a true pairwise match implies equal canonical stems, and the verification guard's behaviour on the spec 0010 cases is unchanged, verifies **AC-7**.
...
4. Move the morphology family to `application/morphology.py` with its two entry points and the agreement property, implement `morphology-v1` to the pinned algorithm and its normative examples, then advance the tokenizer to `lexical-tokenizer-v2`. Then record the step 3 stage level figures, satisfies **AC-6**, **AC-7**, **AC-11**.
