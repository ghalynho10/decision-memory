...
from decision_memory.application.adapter import AdaptOutcome, Manifest, adapt_corpus
...
class EvaluationRunner:
    ...
    def proposed_record_ids(self) -> frozenset[str]:
        ...
        from the records themselves instead of a hardcoded id.
        """
        ids: set[str] = set()
        for path in self.records_dir.glob("*.md"):
            parsed = parse_record_file(path)
            if parsed.record is not None and parsed.record.status == Status.PROPOSED:
                assert parsed.record.id is not None
    ...
    def run_reingest(self, record_id: str, rationale_relpath: str) -> ReingestEvidence:
        ...
        the user's real corpus is never mutated. The assertion passes only
        when the target record's active chunk ids change after the edit, the
        cheapest proof that the multi file fingerprint rule reached the index.
        """
        ...
        with tempfile.TemporaryDirectory(prefix="decision-memory-evaluate-") as tmp:
            ...
            if getattr(ingest_outcome, "exit_code", 1) != 0:
            ...
            if getattr(ingest_again, "exit_code", 1) != 0:
